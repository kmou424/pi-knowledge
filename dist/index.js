...
let disposePromise;
function runtimeModule(modulePath) {
...
async function loadRuntime() {
    ...
    runtimePromise ??= Promise.all([
        import(__rewriteRelativeImportExtension(runtimeModule("./src/engine"))),
        import(__rewriteRelativeImportExtension(runtimeModule("./src/embedding/provider"))),
        import(__rewriteRelativeImportExtension(runtimeModule("./src/storage/sqlite"))),
        import(__rewriteRelativeImportExtension(runtimeModule("./src/watcher/file-watcher"))),
...
function getAuthStorage(ctx) {
    ...
    if (!authStorage || typeof authStorage !== "object")
        return undefined;
    const getApiKey = authStorage.getApiKey;
    if (typeof getApiKey !== "function")
        return undefined;
    return authStorage;
}
function configureEmbeddingAuth(loaded, ctx) {
...
export default function (pi) {
    ...
    if (AUTO_INJECT) {
        pi.on("context", async (event, ctx) => {
            try {
                ...
                return {
                    messages: [{ role: "user", content: `[Knowledge context]\n${context}` }, ...event.messages],
                };
            }
            catch {
                /* silent fail */
    ...
    pi.registerTool({
        ...
        description: "Inspect an indexing source without writing a KB, showing scannable files, suggested exclusions, and technical skips",
        ...
        parameters: Type.Object({
            ...
            include_suggested_text: Type.Optional(Type.Boolean({
                description: "Preview the scope if suggested-excluded text such as vendor/build/runtime/cache/secret-named text is included",
            })),
            include_paths: Type.Optional(Type.Array(Type.String({
                description: "Relative paths under a directory source to include even when they match suggested-exclude patterns",
            }))),
            exclude_paths: Type.Optional(Type.Array(Type.String({ description: "Relative paths under a directory source to exclude from this plan" }))),
        }),
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            include_suggested_text: Type.Optional(Type.Boolean({
                description: "Include text files that are normally suggested for exclusion, such as vendor/build/runtime/cache/secret-named text, after user confirmation",
            })),
            include_paths: Type.Optional(Type.Array(Type.String({
                description: "Relative paths under a directory source to include even when they match suggested-exclude patterns",
            }))),
            exclude_paths: Type.Optional(Type.Array(Type.String({ description: "Relative paths under a directory source to exclude from this KB" }))),
        }),
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            diagnostics: Type.Optional(Type.Boolean({ description: "Include ranking diagnostics and mode/fallback details in the result" })),
        }),
        async execute(_id, params, _signal, _onUpdate, ctx) {
            const { engine } = await ensureInitialized(ctx);
            ...
            if (response.mode_used)
                output += ` — mode: ${response.mode_used}`;
            if (response.retry_modes?.length)
                output += ` — retried: ${response.retry_modes.join(", ")}`;
            output += ":\n\n";
            ...
            output += response.results
                .map((r, i) => {
                const diag = diagnostics && r.ranking
                    ? `\nDiagnostics: base=${r.ranking.base_score.toFixed(3)}, adjusted=${r.ranking.adjusted_score.toFixed(3)}, coverage=${r.ranking.coverage.toFixed(2)}, path_boost=${r.ranking.path_boost.toFixed(2)}, source_boost=${r.ranking.source_boost.toFixed(2)}, test=${r.ranking.is_test}`
                    : "";
    ...
    pi.registerTool({
        ...
        async execute(_id, _params, _signal, _onUpdate, ctx) {
            ...
            for (const kb of kbs) {
                ...
                if (kb.source_path)
                    lines.push(`    source: ${kb.source_path}`);
                if (diag) {
                    if (diag.job) {
                        ...
                        const progress = [
                            ...
                            .filter(Boolean)
                            .join(", ");
                        lines.push(`    job: ${diag.job.status}/${diag.job.phase} — ${progress || "no processed items yet"} — last progress ${jobAge}s ago`);
                        if (diag.job.message)
                            lines.push(`    last: ${diag.job.message}`);
                        if (diag.job.error_message)
                            lines.push(`    error: ${diag.job.error_message}`);
                    }
                    ...
                    if (diag.stuck_indexing)
                        lines.push(`    ⚠️ indexing appears stuck for ${Math.round(diag.last_progress_age_ms / 60000)}m; remove and rebuild if no pi process is actively indexing it`);
                    if (diag.skipped_files.total > 0)
                        lines.push(`    skipped: ${diag.skipped_files.total} files (${Object.entries(diag.skipped_files.by_reason)
            ...
            if (totalStale === 0 && totalOrphans === 0 && totalStuck === 0 && kbs.length > 0)
                ...
                lines.push("", `Health: ⚠️ ${totalStale} stale, ${totalOrphans} orphans, ${totalStuck} stuck indexing — run knowledge_update or rebuild affected KBs`);
    ...
    pi.registerTool({
        ...
        async execute(_id, _params, _signal, _onUpdate, ctx) {
            ...
            const kbs = engine.list();
            if (kbs.length === 0)
                return { content: [{ type: "text", text: "No knowledge bases." }] };
