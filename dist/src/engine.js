...
import { diagnoseKB } from "./diagnostics/health.js";
import { dispose as disposeEmbedding, embedDocuments, embedQuery, getCurrentEmbeddingModel, prepareForShutdown as prepareEmbeddingForShutdown, } from "./embedding/provider.js";
import { openVectorReader, openVectorWriter } from "./embedding/vectors.js";
import { buildChunkEmbeddingText, chunkFile, chunkIdentityHash, createSkippedScanStats, isReadableTextFile, iterateScannableFiles, iterateScannedFiles, preTokenizeForFTS, summarizeSkippedScan, } from "./indexer/chunker.js";
import { shutdownModelWorker } from "./model-worker-client.js";
import { searchBM25 } from "./search/bm25.js";
import { weightedScoreFusion } from "./search/fusion.js";
import { normalizedQueryText, tokenizeForSearch } from "./search/query.js";
import { hasAnyLexicalEvidence, hasEnoughLexicalEvidence, MIN_HYBRID_SCORE, normalizeFileTypeFilter, queryCoverage, scoreChunkForQuery, } from "./search/ranking.js";
import { disposeReranker, prepareRerankerForShutdown, rerank } from "./search/reranker.js";
import { searchVectorFile } from "./search/vector.js";
import { createKB, deleteChunksByIds, deleteKB, finishIndexingJob, getChunkById, getChunkCount, getChunksByFile, getChunksByKB, getFileCount, getKB, getKBByName, insertChunks, iterateChunkIdsByKB, iterateChunksByKB, listKBs, openDatabase, startIndexingJob, updateIndexingJob, updateKBCounts, updateKBStatus, } from "./storage/sqlite.js";
...
function tempVectorPath(vectorPath) {
...
function serializeAddOptions(options = {}) {
    const normalized = {};
    if (options.include_suggested_text === true)
        normalized.include_suggested_text = true;
    if (options.include_paths && options.include_paths.length > 0)
        normalized.include_paths = options.include_paths;
    if (options.exclude_paths && options.exclude_paths.length > 0)
        normalized.exclude_paths = options.exclude_paths;
    return Object.keys(normalized).length > 0 ? JSON.stringify(normalized) : undefined;
}
function parseAddOptions(raw) {
    if (!raw)
        return {};
    try {
...
function planDirectoryScan(dirPath, options = {}) {
...
function lineProximity(a, b) {
    if (a.chunk.file_path !== b.chunk.file_path || a.chunk.kb_id !== b.chunk.kb_id)
        return 0;
    if (a.sourceChunkIds.some((id) => b.sourceChunkIds.includes(id)))
        return 1;
...
function diversifyRankedChunks(candidates, diversity, vectorsByChunkId = new Map()) {
    if (diversity === "off" || candidates.length <= 2)
        return candidates;
    const lambda = diversity === "strong" ? 0.62 : 0.76;
    const normalized = normalizeScores(candidates);
    const tokenSets = new Map(candidates.map((candidate) => [candidate.chunk.id, tokenizeForSimilarity(candidate.content)]));
    const selected = [];
    ...
    while (remaining.length > 0) {
        ...
        for (let i = 0; i < remaining.length; i++) {
            ...
            for (const chosen of selected) {
                const lexical = jaccardSimilarity(tokenSets.get(candidate.chunk.id) ?? new Set(), tokenSets.get(chosen.chunk.id) ?? new Set());
                const vector = Math.max(0, cosineSimilarity(vectorsByChunkId.get(candidate.chunk.id), vectorsByChunkId.get(chosen.chunk.id)));
                redundancy = Math.max(redundancy, Math.max(lexical, lineProximity(candidate, chosen), vector * VECTOR_REDUNDANCY_WEIGHT));
            }
...
function interleaveByFile(candidates, diversity) {
    if (diversity === "off" || candidates.length <= 2)
        return candidates;
...
function buildQuerySnippet(content, query, maxLength = 240) {
...
function buildAdaptiveContext(seed, chunks, queryTokens) {
    const scored = chunks
        .map((chunk) => {
        const distance = chunk.id === seed.id
            ? 0
    ...
    if (!scored.some((item) => item.chunk.id === seed.id))
        scored.push({ chunk: seed, score: Number.MAX_SAFE_INTEGER });
    ...
    for (const chunk of ordered) {
        if (totalLength >= ADAPTIVE_MAX_CONTEXT_CHARS)
            break;
...
function pushAdaptiveCandidate(candidates, candidate) {
    const overlappingIndex = candidates.findIndex((existing) => existing.chunk.kb_id === candidate.chunk.kb_id &&
...
function isExactLookupQuery(query) {
    const trimmed = query.trim();
    if (/["'`]/.test(trimmed))
        return true;
    if (/[./\\][\w.-]+/.test(trimmed))
        return true;
    if (/\b[A-Z][A-Za-z0-9_]*[A-Z][A-Za-z0-9_]*\b/.test(trimmed))
        return true;
    if (/\b[A-Z]{2,}[_-]?[A-Z0-9]*\b/.test(trimmed))
        return true;
    if (/\b[a-zA-Z_][\w-]*\.(ts|tsx|js|jsx|go|rs|py|java|md|json|ya?ml|toml)\b/.test(trimmed))
        return true;
    if (/\b[A-Z]+-\d+\b/.test(trimmed))
        return true;
    return false;
}
function chooseAutoMode(query) {
    const normalized = query.trim();
    if (isExactLookupQuery(normalized))
        return "fast";
...
function isWeakAutoResponse(query, response, primaryMode, attemptMode) {
    ...
    if (primaryMode === "fast") {
        const exactNeedle = query
            ...
            .replace(/^["'`]|["'`]$/g, "")
            .toLowerCase();
        const hasExactHit = response.results.some((result) => result.file_path.toLowerCase().includes(exactNeedle) || result.content.toLowerCase().includes(exactNeedle));
        if (!hasExactHit && response.results.every((result) => (result.ranking?.coverage ?? 0) < 0.67))
            return true;
    }
...
async function chunkUrl(source, signal) {
...
export class KnowledgeEngine {
    ...
    plan(source, options = {}) {
        ...
        if (isFile) {
            const skipped = createSkippedScanStats();
            const supportedDocument = resolvedSource.endsWith(".pdf") || resolvedSource.endsWith(".docx") || resolvedSource.endsWith(".doc");
    ...
    async add(source, name, onProgress, signal, options = {}) {
        if (!this.db)
        ...
        if (existingKB) {
            throw new Error(`Knowledge base "${name}" already exists. Use knowledge_update to refresh it, or knowledge_remove before adding a replacement.`);
        }
        ...
        try {
            ...
            const reportProgress = (phase, processedFiles, totalFiles, skippedTotal = 0) => {
                ...
                if (skippedTotal > 0)
                    suffix = `${suffix}, skipped ${skippedTotal}`;
            ...
            const flushPending = async (processedFiles, totalFiles) => {
                ...
                const vectors = await embedDocuments(batch.map((chunk) => buildChunkEmbeddingText(chunk)), signal);
                if (signal?.aborted)
                    throw new Error("Cancelled");
            ...
            const addChunks = async (chunks, processedFiles, totalFiles) => {
            ...
            if (isUrl) {
                ...
                const planningMessage = `Planned directory scan: ${plan.files} files, ${formatBytes(plan.bytes)} scannable text, skipped ${plan.skippedTotal} (${plan.skippedSummary})`;
                ...
                const finalizingMessage = `Scanned ${processedFiles} files, skipped ${skipped.total} (${summarizeSkippedScan(skipped)}), finalizing...`;
                updateIndexingJob(this.db, kb.id, {
                    phase: "finalizing",
            ...
            if (!savedKB)
                throw new Error(`Knowledge base disappeared after add: ${kb.id}`);
            const readyMessage = `Ready: ${chunkCount} chunks from ${savedFileCount} files in ${formatDuration(Date.now() - startedAt)}`;
            updateIndexingJob(this.db, kb.id, {
                phase: "ready",
            ...
            vectorWriter?.close();
            if (tempVectorFile)
                rmSync(tempVectorFile, { force: true });
            if (this.db) {
                finishIndexingJob(this.db, kb.id, isCancellationError(e) ? "cancelled" : "failed", isCancellationError(e) ? "Indexing cancelled." : "Indexing failed.", e instanceof Error ? e.message : String(e));
            }
    ...
    async update(nameOrId, onProgress, signal) {
        if (!this.db)
            throw new Error("Engine not initialized");
        const kb = getKB(this.db, nameOrId) ?? getKBByName(this.db, nameOrId);
        if (!kb)
            throw new Error(`Knowledge base not found: ${nameOrId}`);
        ...
        const updateableKB = { ...kb, source_path: kb.source_path };
        ...
        const updateRun = this.runUpdate(updateableKB, onProgress, signal).finally(() => {
            if (this.activeUpdates.get(updateableKB.id) === updateRun)
                this.activeUpdates.delete(updateableKB.id);
        });
        this.activeUpdates.set(updateableKB.id, updateRun);
        return updateRun;
    }
    async runUpdate(kb, onProgress, signal) {
        if (!this.db)
            throw new Error("Engine not initialized");
        ...
        let replacementVectorPath;
        let addedVectorPath;
        let addedVectorWriter;
        try {
            const vectorPath = join(this.knowledgeDir, "vectors", `${kb.id}.bin`);
            ...
            const oldVectorIndexByHash = new Map();
            const newVectorIndexByHash = new Map();
            const pendingChunks = [];
            ...
            let scannedFiles = 0;
            let scannedChunks = 0;
            let plannedTotalFiles;
            const startedAt = Date.now();
            const flushPending = async () => {
                if (!this.db || !addedVectorWriter || pendingChunks.length === 0)
                    return;
                if (signal?.aborted)
                    throw new Error("Cancelled");
                const batch = pendingChunks.splice(0, INDEX_EMBED_BATCH_SIZE);
                const elapsed = Date.now() - startedAt;
                const message = `Embedding update batch: ${addedCount} new chunks stored, ${scannedFiles} files scanned, elapsed ${formatDuration(elapsed)}`;
                updateIndexingJob(this.db, kb.id, {
                    phase: "embedding",
                    message,
                ...
                onProgress?.(message);
                const newVectors = await embedDocuments(batch.map((c) => buildChunkEmbeddingText(c)), signal);
                if (signal?.aborted)
                    throw new Error("Cancelled");
            ...
            const processChunks = async (chunks) => {
                for (const chunk of chunks) {
                    ...
                    if (pendingChunks.length >= INDEX_EMBED_BATCH_SIZE)
                        await flushPending();
                }
            };
            ...
            if (kb.source_type === "url") {
                ...
                const message = `Scanned ${scannedFiles} files, skipped ${skipped.total} (${summarizeSkippedScan(skipped)}), reconciling deletes...`;
                updateIndexingJob(this.db, kb.id, {
                    phase: "reconciling",
                    message,
                    processed_files: scannedFiles,
            ...
            const takeVectorIndex = (indexesByHash, hash) => {
            ...
            try {
                for (const chunk of iterateChunksByKB(this.db, kb.id)) {
                    ...
                    const vector = oldVectorReader && oldVectorIndex !== undefined
                        ? oldVectorReader.read(oldVectorIndex)
                        : newVectorReader && newVectorIndex !== undefined
                            ? newVectorReader.read(newVectorIndex)
                            : undefined;
                    if (!vector)
                        throw new Error(`Missing vector while rebuilding knowledge base: ${chunk.id}`);
            ...
            if (addedVectorPath)
                rmSync(addedVectorPath, { force: true });
            ...
            addedVectorWriter?.close();
            if (replacementVectorPath)
                rmSync(replacementVectorPath, { force: true });
            if (addedVectorPath)
                rmSync(addedVectorPath, { force: true });
            updateKBStatus(this.db, kb.id, "error");
            finishIndexingJob(this.db, kb.id, isCancellationError(e) ? "cancelled" : "failed", isCancellationError(e) ? "Update cancelled." : "Update failed.", e instanceof Error ? e.message : String(e));
            throw e;
        }
    }
    async search(query, options = {}) {
        ...
        const availableKBs = kb_id ? [selectedKB].filter(Boolean) : listKBs(db);
        ...
        for (const kb of kbs) {
            ...
            if (retrievalMode === "fast") {
                ...
                for (const [chunkId, vector] of vectorResults.vectorsByChunkId)
        ...
        const rankingByChunkId = new Map();
        ...
        if (normalizedFileType || filters?.path_pattern) {
            filtered = scored.filter((r) => {
                ...
                if (!chunk)
                    return false;
                if (normalizedFileType && chunk.file_type !== normalizedFileType)
                    return false;
                if (filters?.path_pattern && !chunk.file_path.includes(filters.path_pattern))
                    return false;
                return true;
            });
        }
        ...
        for (const r of filtered) {
            ...
            if (mode === "adaptive") {
                const contextChunks = getChunksByFile(db, chunk.kb_id, chunk.file_path, Math.max(1, chunk.start_line - ADAPTIVE_CONTEXT_LINES), chunk.end_line + ADAPTIVE_CONTEXT_LINES);
        ...
        for (const diagnostic of diagnostics) {
            if (diagnostic.stuck_indexing) {
                ...
                issues.push({
                    ...
                    action: "Check knowledge_status for the last progress message. If no Pi process is actively indexing it, remove and rebuild this KB.",
                });
            }
            ...
            if (diagnostic.skipped_files.total > 0) {
                issues.push({
                    severity: "info",
                    kb_name: diagnostic.kb_name,
                    message: `${diagnostic.skipped_files.total} files were skipped while scanning (${Object.entries(diagnostic.skipped_files.by_reason)
                        .filter(([, count]) => count > 0)
                        .map(([reason, count]) => `${reason}: ${count}`)
                        .join(", ")}).`,
                    action: "Use skipped samples to confirm exclusions are expected. Adjust source path or ignore rules if needed.",
                });
            }
        }
    ...
    async exportKB(nameOrId, outputPath) {
        if (!this.db)
            throw new Error("Engine not initialized");
        const kb = getKB(this.db, nameOrId) ?? getKBByName(this.db, nameOrId);
        if (!kb)
    ...
    async importKB(inputPath, onProgress, signal) {
        if (!this.db)
            throw new Error("Engine not initialized");
        const { readFileSync } = await import("node:fs");
        ...
        try {
            ...
            for (let offset = 0; offset < indexedChunks.length; offset += INDEX_EMBED_BATCH_SIZE) {
                ...
                const message = `Embedding import batch ${Math.floor(offset / INDEX_EMBED_BATCH_SIZE) + 1}/${Math.ceil(indexedChunks.length / INDEX_EMBED_BATCH_SIZE)}: ${offset}/${indexedChunks.length} chunks`;
                updateIndexingJob(this.db, kb.id, {
                    phase: "embedding",
                    message,
                ...
                onProgress?.(message);
                const vectors = await embedDocuments(batch.map((c) => buildChunkEmbeddingText(c)), signal);
                if (signal?.aborted)
                    throw new Error("Cancelled");
                insertChunks(this.db, kb.id, batch);
                vectorWriter.append(vectors);
                inserted += batch.length;
                updateKBCounts(this.db, kb.id, inserted, new Set(indexedChunks.slice(0, inserted).map((c) => c.file_path)).size);
                updateIndexingJob(this.db, kb.id, {
                    phase: "storing",
            ...
            const savedKB = getKB(this.db, kb.id);
            if (!savedKB)
                throw new Error(`Knowledge base disappeared after import: ${kb.id}`);
            ...
            if (tempVectorFile)
                rmSync(tempVectorFile, { force: true });
            if (this.db) {
                finishIndexingJob(this.db, kb.id, isCancellationError(e) ? "cancelled" : "failed", isCancellationError(e) ? "Import cancelled." : "Import failed.", e instanceof Error ? e.message : String(e));
            }
            deleteKB(this.db, kb.id);
            throw e;
        }
    ...
    async dispose(options = {}) {
        ...
        if (activeUpdates.length > 0)
            await Promise.allSettled(activeUpdates);
