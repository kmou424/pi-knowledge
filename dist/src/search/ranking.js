import { buildChunkEmbeddingText } from "../indexer/chunker.js";
import { signalTokens, tokenizeForSearch } from "./query.js";
...
export function normalizeFileTypeFilter(fileType) {
    if (!fileType)
        return undefined;
...
export function queryAsksForTests(queryTokens) {
    const signals = signalTokens(queryTokens);
    return ["assert", "fixture", "spec", "test", "testing"].some((token) => signals.has(token));
}
function queryAsksForUserGuide(queryTokens) {
    const signals = signalTokens(queryTokens);
    return ["config", "configuration", "install", "onboard", "quickstart", "setup", "start", "usage"].some((token) => signals.has(token));
}
function queryAsksForLocalization(queryTokens) {
    const signals = signalTokens(queryTokens);
    return ["i18n", "lang", "language", "locale", "localization", "translation"].some((token) => signals.has(token));
}
export function isLocalizationPath(filePath, fileType) {
    if (!["json", "markdown", "text", "toml", "yaml"].includes(fileType))
        return false;
    const lower = filePath.toLowerCase();
    return (lower.includes("/locale/") ||
...
export function hasAnyLexicalEvidence(text, queryTokens) {
    return queryCoverage(text, queryTokens) > 0;
}
export function pathTokenBoost(filePath, queryTokens) {
...
export function basenameTokenBoost(filePath, queryTokens) {
    ...
    const signals = signalTokens(queryTokens);
    if (baseTokens.size === 0 || signals.size === 0)
        return 0;
    for (const token of signals) {
...
export function sourceFileBoost(chunk, queryTokens) {
    if (!SOURCE_FILE_TYPES.has(chunk.file_type))
        return 0;
    if (queryAsksForUserGuide(queryTokens))
        return Math.min(0.05, pathTokenBoost(chunk.file_path, queryTokens));
...
function documentationBoost(chunk, queryTokens) {
    if (!queryAsksForUserGuide(queryTokens))
        return 0;
...
export function scoreChunkForQuery(baseScore, chunk, queryTokens) {
    const guideIntent = queryAsksForUserGuide(queryTokens);
    const pathBoost = guideIntent && SOURCE_FILE_TYPES.has(chunk.file_type)
...
export function hasEnoughLexicalEvidence(chunk, queryTokens) {
    const signals = signalTokens(queryTokens);
    if (signals.size <= 1)
        return true;
    if (sourceFileBoost(chunk, queryTokens) >= 0.22)
        return true;
    const coverage = queryCoverage(buildChunkEmbeddingText(chunk), queryTokens);
    if (signals.size <= 3)
        return coverage >= 1 / signals.size;
