import { buildChunkEmbeddingText, contentHash, preTokenizeForFTS } from "../chunker.js";
const LANGS = {
    typescript: {
        grammar: async () => {
...
function collectChunks(root, config) {
    const chunks = [];
    function walk(node) {
...
export async function chunkWithAST(content, filePath, language) {
    ...
    const fns = collectChunks(tree.rootNode, config);
    if (fns.length === 0)
        return [];
    return fns.map((fn) => {
        const chunk = {
