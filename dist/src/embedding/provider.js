...
export function setEmbeddingApiKeyResolver(resolver) {
...
export async function prepareForShutdown() {
    clearIdleTimer();
    await waitForNoActiveRuns();
}
async function embedViaAPI(texts, prefix) {
    ...
    const maxChars = Number.isFinite(configuredMaxChars) && configuredMaxChars > 0 ? configuredMaxChars : DEFAULT_API_MAX_EMBED_CHARS;
    ...
    if (provider === "openai") {
        ...
        if (!apiKey) {
            throw new Error("PI_KNOWLEDGE_EMBEDDING_API_KEY or auth.json credential 'pi-knowledge' required for openai embedding");
        }
        const baseUrl = process.env.PI_KNOWLEDGE_EMBEDDING_BASE_URL?.trim() ||
...
export async function embedTexts(texts, prefix, signal) {
    if (!EMBEDDING_CONFIG.startsWith("local")) {
        try {
            return await embedViaAPI(texts, prefix);
        }
        catch (error) {
            if (!API_FALLBACK_TO_LOCAL)
                throw error;
            console.warn(`pi-knowledge: embedding API failed; falling back to local model because PI_KNOWLEDGE_EMBEDDING_API_FALLBACK=local (${error instanceof Error ? error.message : String(error)})`);
        }
    }
    beginRun();
    try {
...
export async function embedDocuments(texts, signal) {
