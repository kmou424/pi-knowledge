...
export function openVectorWriter(path) {
    ...
    return {
        append(vectors) {
            if (closed)
                throw new Error("Vector writer is already closed");
...
export function openVectorReader(path) {
    if (!existsSync(path))
        return undefined;
    ...
    return {
        ...
        readInto(index, target) {
            if (closed || index < 0 || index >= count || dim === 0 || target.length !== dim)
                return false;
            const bytesRead = readSync(fd, readBuffer, 0, vectorBytes, 8 + index * vectorBytes);
            if (bytesRead < vectorBytes)
                return false;
            for (let i = 0; i < dim; i++)
                target[i] = readBuffer.readFloatLE(i * 4);
            return true;
        },
        close() {
            if (closed)
...
export function saveVectors(path, vectors) {
